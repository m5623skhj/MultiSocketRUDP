...
// @details std::atomic을 사용하여 세션 상태를 원자적으로 관리하고, 명시적인 상태 전이 함수들을 제공합니다.
// ----------------------------------------
...
public:
	...
	// ----------------------------------------
	// @brief 예약 상태를 연결 상태로 전환하려 시도합니다.
	// @return RESERVED에서 CONNECTED로 전환한 경우 true를 반환합니다.
	// ----------------------------------------
    [[nodiscard]]
    bool TryTransitionToConnected() noexcept;
	// ----------------------------------------
	// @brief 예약 또는 연결 상태를 해제 중 상태로 전환하려 시도합니다.
	// @return RESERVED 또는 CONNECTED에서 RELEASING으로 전환한 경우 true를 반환합니다.
	// ----------------------------------------
    [[nodiscard]]
    bool TryTransitionToReleasing() noexcept;
	// ----------------------------------------
	// @brief 아직 연결되지 않은 예약 세션을 해제 중 상태로 전환하려 시도합니다.
	// @return RESERVED에서 RELEASING으로 전환한 경우 true를 반환합니다.
	// ----------------------------------------
    [[nodiscard]]
    bool TryAbortReserved() noexcept;
	// ----------------------------------------
	// @brief 세션 상태를 연결 해제로 설정합니다.
	// ----------------------------------------
