...
`RUDPPacketProcessor::OnRecvPacket`에서 본 처리 전 세 가지 조건을 확인한다.
...
`NetBuffer` 해제는 이 함수가 아니라 호출자인 `MultiSocketRUDPCore::OnRecvPacket()`에서 수행한다.

**`GetPayloadLength` 동작:**

```cpp
// 헤더 구조: [HeaderCode 1B][PayloadLength 2B][Reserved 2B]  (총 5 bytes = df_HEADER_SIZE)
//             offset 0        offset 1-2          offset 3-4
...
| 그 외 RIO Status≠0 | `BY_ERROR` 사유로 세션 종료 | ✅ |
