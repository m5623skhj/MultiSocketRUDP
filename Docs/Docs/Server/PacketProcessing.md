...
```cpp
WORD GetPayloadLength(const NetBuffer& buffer) {
    // [HeaderCode 1B][Reserved 1B][PayloadLength 2B][...payload...]
    //  offset 0        offset 1    offset 2-3
    WORD payloadLen;
    memcpy(&payloadLen, buffer.m_pSerializeBuffer + 2, sizeof(WORD));
    return payloadLen + HEADER_SIZE;  // 전체 패킷 크기
}
```
...
void RUDPPacketProcessor::ProcessByPacketType(
    ...
    BYTE packetType)
{
    auto& clientAddr = *reinterpret_cast<sockaddr_in*>(clientAddrBuffer.data());
...
bool RUDPSession::OnRecvPacket(NetBuffer& recvPacket)
{
    ...
    switch (result) {
    case PacketOrdererResult::PROCESSED:
        return true;

    case PacketOrdererResult::DUPLICATED_RECV:
        // 이미 처리한 패킷 → 재ACK만 전송
        SendReplyToClient(packetSequence);
        return true;

    case PacketOrdererResult::PACKET_HELD:
        // 순서 맞는 패킷 아직 안 옴 → HoldingQueue에 보관
        return true;

    case PacketOrdererResult::ERROR_OCCURED:
...
```cpp
PacketOrdererResult SessionPacketOrderer::OnReceive(
    PacketSequence sequence,
    NetBuffer* buffer,
    const ProcessPacketFunc& processFunc)
{
    if (sequence == nextExpectedSequence) {
        // ① 정상 순서 패킷 → 즉시 처리
        if (!processFunc(buffer, sequence)) return ERROR_OCCURED;
        ++nextExpectedSequence;

        // ② HoldingQueue에서 연속으로 처리 가능한 패킷 처리
        return ProcessHoldingPacket(processFunc);

    } else if (sequence > nextExpectedSequence) {
        // ③ 미래 시퀀스 → HoldingQueue에 보관
        if (holdingQueue.size() >= maxHoldingQueueSize) {
            LOG_ERROR(std::format("Holding queue full. maxSize={}", maxHoldingQueueSize));
            return ERROR_OCCURED;
        }
        holdingQueue.emplace(sequence, buffer);
        return PACKET_HELD;

    } else {
        // ④ 과거 시퀀스 → 중복 패킷
        return DUPLICATED_RECV;
    }
}

PacketOrdererResult SessionPacketOrderer::ProcessHoldingPacket(
    const ProcessPacketFunc& processFunc)
{
    // HoldingQueue (min-heap, 시퀀스 오름차순)에서 순서대로 처리
    while (!holdingQueue.empty()) {
        auto& [topSeq, topBuf] = holdingQueue.top();
        if (topSeq != nextExpectedSequence) break;  // 아직 이전 패킷 없음

        holdingQueue.pop();
        if (!processFunc(topBuf, topSeq)) return ERROR_OCCURED;
        ++nextExpectedSequence;
    }
    return PROCESSED;
}
