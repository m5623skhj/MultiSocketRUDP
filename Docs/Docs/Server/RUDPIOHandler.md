...
2. [DoRecv — 수신 등록](#2-dorecv--수신-등록)
3. [DoSend — 송신 시도 (SpinLock 방식)](#3-dosend--송신-시도-spinlock-방식)
4. [MakeSendStream — 배치 전송 스트림 구성](#4-makesendstream--배치-전송-스트림-구성)
5. [IOCompleted — 완료 처리 분기](#5-iocompleted--완료-처리-분기)
6. [RecvIOCompleted — 수신 완료](#6-recviocompletd--수신-완료)
7. [SendIOCompleted — 송신 완료](#7-sendiocompletd--송신-완료)
8. [RefreshRetransmissionSendPacketInfo](#8-refreshretransmissionsendpacketinfo)
...
```cpp
{
    ...
    if (!MakeSendStream(session, threadId)) {
...
## 4. MakeSendStream — 배치 전송 스트림 구성

```cpp
bool RUDPIOHandler::MakeSendStream(
...
```cpp
{
    ...
    for (auto* info : pendingInfos) {
        ...
        // ③ 재전송 타임스탬프 갱신
        RefreshRetransmissionSendPacketInfo(
            *info, threadId,
            GetTickCount64() + retransmissionMs);
    ...
    // ④ RIO_BUF 구성 + RIOSend 호출
    RIO_BUF sendRIOBuf = {
        sendCtx.GetSendBufferId(),  // 등록된 버퍼 ID
        0,                          // 오프셋
        static_cast<ULONG>(offset)  // 전송할 총 크기
    };

    std::shared_lock socketLock(session.GetSocketContext().GetSocketMutex());
    if (session.GetSocketContext().GetSocket() == INVALID_SOCKET) {
        sendCtx.ioMode.store(IO_MODE::IO_NONE_SENDING, std::memory_order_release);
        return false;
    }

    rioManager.RIOSend(
        sendCtx.GetSendRIORQ(),
        &sendRIOBuf, 1,
        0,  // 플래그
        reinterpret_cast<void*>(&session)  // RequestContext
    );
...
## 8. RefreshRetransmissionSendPacketInfo
