...
AI 리뷰 status는 필수 체크로 사용하지 않는다. `ai-review/gemini`은 Gemini API의 일시적 503, rate limit, 응답 형식 실패 같은 외부 서비스 상태에 영향을 받으므로 PR merge gate가 아니라 보조 리뷰 신호로만 사용한다.
...
IntegrationClientHarness
  ...
  - connect, reserve-timeout, echo, drop-ack 시나리오 실행
```
...
`RunReserveOnlyScenario`와 `RunDropAckScenario`는 정상 사용 흐름이 아니라 서버의 timeout/retransmission 처리를 검증하기 위한 부정 시나리오다.
