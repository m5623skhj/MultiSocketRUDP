...
> 기존 통합 문서의 상세 코드 해설과 운영 메모를 보존한 레퍼런스다.
> 일반적인 실행 경로는 상위 [[Testing|테스트 허브]]에서 목적에 맞는 문서를 먼저 선택한다.
...
| `ActionExecutionTests.cs` | action chain·분기·순환 차단, packet wait, 변수·assert·random/custom action 실행 |
...
- `ping`
  - 등록된 애플리케이션 packet handler를 통해 Ping을 처리하고 Pong이 왕복되는지 검증
- `drop-ack`
...
`BotTester.yml`은 C# consumer만 실행한다. 공용 vector가 변경되면 `CI.yml`이 Native GTest와 BotTester workflow를 모두 호출해 양쪽을 검증한다.
...
IntegrationClientHarness
  ...
  - connect, reserve-timeout, echo, ping, drop-ack, disconnect, stop, multi-echo, ordered-burst 시나리오 실행
```
...
| `RunPingScenario` | Ping을 보내고 등록된 애플리케이션 handler가 반환한 Pong을 기다린다. |
