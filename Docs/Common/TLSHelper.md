...
```cpp
[[nodiscard]]
TlsDecryptResult DecryptDataStream(std::vector<char>& encryptedStream, OUT char* plainBuffer, OUT size_t& plainSize);
```
...
테스트 서버를 구동하고 세션 팩토리를 등록한다. 내부적으로 `RegisterContentsPacketsOnce`를 호출하여 패킷을 등록하며, `IntegrationSession`을 생성하는 팩토리를 사용하여 서버를 시작한다.
...
누적된 TLS 바이트 스트림에서 완성된 레코드만 복호화한다. 반환값을 무시하면 컴파일 경고가 발생한다. 호출 측에서 반드시 검사해야 한다.

| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `encryptedStream` | `std::vector<char>&` | 복호화할 데이터가 포함된 암호화 스트림 |
| `plainBuffer` | `char*` | [출력] 복호화된 데이터를 저장할 버퍼 |
| `plainSize` | `size_t&` | [출력] 복호화된 데이터의 크기 |
