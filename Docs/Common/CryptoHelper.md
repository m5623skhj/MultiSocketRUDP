...
5. [Nonce 생성 — FillNonce](#5-nonce-생성--fillnonce)
...
## 5. Nonce 생성 — `FillNonce`

```cpp
static bool CryptoHelper::FillNonce(
    const unsigned char* sessionSalt,
    const size_t sessionSaltSize,    // SESSION_SALT_SIZE = 16
    PacketSequence packetSequence,
    PACKET_DIRECTION direction,
    unsigned char* outNonce,
    const size_t nonceSize           // NONCE_SIZE = 12
)
```
...
호출자가 12바이트 버퍼를 제공하므로 nonce 생성 과정에서 동적 할당이 발생하지 않는다. 인자 크기나 포인터가 잘못되면 `false`를 반환한다.
...
static bool CryptoHelper::EncryptAESGCM(
    ...
    const unsigned char* aad,     size_t aadSize,         // 14 bytes
...
unsigned char aad[14] = {};  // 5B 헤더 + type + sequence
...
#### `static bool FillNonce(const unsigned char* sessionSalt, const size_t sessionSaltLen, const PacketSequence packetSequence, const PACKET_DIRECTION direction, unsigned char* outNonce, const size_t nonceSize)`
- 세션 솔트, 패킷 시퀀스, 방향 비트로 호출자가 제공한 버퍼에 GCM nonce를 기록한다.
- 솔트 또는 nonce 크기가 현재 프로토콜 상수와 다르면 `false`를 반환한다.
